...
function EventList() {
	...
	const columns = [
        ...
        {
            name: 'Date début',
            selector: row => row.begin,
            sortable: true,
            cell: row => <div style={{display: 'block'}}>{row.begin}</div>
        },
        {
            name: 'Date fin',
            selector: row => row.end,
            sortable: true,
            cell: row => <div style={{display: 'block'}}>{row.end}</div>
        },
        {
            name: 'Actions',
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                showLoader()
                axios.delete(`/api/events/remove/${id}`)
                .then(function (response) {
                        ...
                        <div className="card mt-3">
                            <div className="card-body p-3">
                                ...
                                <DataTable 
                                    ...
                                    noDataComponent={<div className="p-2">Aucun évènement trouvée.</div>}
                                />
                            </div>
                        </div>
                    </div>
