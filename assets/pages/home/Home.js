...
function Home() {
    ...
    const columns = [
        ...
        {
            name: 'Image',
            selector: row => row.imageFile,
            sortable: false,
            cell: row => <div style={{display: 'block', minWidth: '120px'}}>{row.imageFile}</div>
        },
        {
            name: 'Titre',
                ...
                doc.imageFile = (
                    <>
                        <a
                            href={doc.imageFile}
                            target="blank">
                            <img src={doc.imageFile} className={`img-thumbnail`} style={{ width: "100px" }} />
                        </a>
                    </>
                )
                doc.actions = (
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                showLoader()
                axios.delete(`/api/home/remove/${id}`)
                .then(function (response) {
                        ...
                        <div className="card mt-3">
                            <div className="card-body p-3">
                                ...
                                <DataTable 
                                    ...
                                    noDataComponent={<div className="p-2">Aucune donnée trouvée.</div>}
                                />
                            </div>
                        </div>
                    </div>
